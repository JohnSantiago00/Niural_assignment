...
Key implementation decisions, alternatives considered, and why the current version fits this product workflow.

## Decision Table

| Decision | What I chose | Production alternative | Why this fits now |
| --- | --- | --- | --- |
| Workflow state | Supabase tables + server-side workflow helpers | Dedicated workflow engine or queue | Sufficient for an end-to-end hiring pipeline without adding another state system. |
...
| AI safety | Zod validation + source-bound prompts | Human review queues for every AI artifact | Keeps AI outputs bounded without slowing every action. |
...
| Calendar integration | One configured Google Calendar | Per-interviewer OAuth | Keeps setup manageable while exercising real Calendar APIs. |
...
| Offer signing | Custom canvas signature pad | DocuSign/PandaDoc/HelloSign | Keeps signing in-app while capturing signature, timestamp, and signer IP. |
| Signature storage | PNG data URL on `offers` | Private storage artifact + immutable signing log | Keeps the signature attached to the offer record; production would move to immutable artifacts. |
...
| Setup | `npm run db:setup` | Supabase CLI project or manual SQL editor | Gives the repo one deterministic database setup path. |
...
A production system would move many of these into jobs. Server actions keep the causal chain explicit in this codebase, while deterministic fallbacks and artifact caching reduce the main reliability risks without adding queue infrastructure.
...
The signing flow uses a lightweight custom UI that captures:

- signature
- timestamp
- IP address

The canvas signature pad satisfies that directly and keeps the user experience inside the app. It also avoids pretending a vendor integration exists when the app owns the signing interaction.
